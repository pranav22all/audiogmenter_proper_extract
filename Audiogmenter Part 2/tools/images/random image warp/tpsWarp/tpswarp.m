...
for j = 1:NWs
    for i = 1:np
        coeff = sqrt((xp(i)-X(j))^2 + (yp(i)-Y(j))^2);
        coeff = 2 * coeff^2 * log(max(coeff,realmin));
        Xw(j) = Xw(j) + coeff*wW(i,1);
        Yw(j) = Yw(j) + coeff*wW(i,2);
    end
    Xw(j) = Xw(j) + wW(np+1,1);
    Xw(j) = Xw(j) + X(j)*wW(np+2,1);
    Xw(j) = Xw(j) + Y(j)*wW(np+3,1);
    
    Yw(j) = Yw(j) + wW(np+1,2);
    Yw(j) = Yw(j) + X(j)*wW(np+2,2);
    Yw(j) = Yw(j) + Y(j)*wW(np+3,2);
end
